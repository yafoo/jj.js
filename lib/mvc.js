...
async function mvc(ctx, next) {
    ...
    if(cfg_app.app_debug) {
        Logger.http('Headers:', ctx.headers);
        Logger.http('Router:', {TYPE: controller_folder, DEEP: ctx.DEEP, CONTROLLER: ctx.CONTROLLER, ACTION: ctx.ACTION});
