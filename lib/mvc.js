...
const {toCamelCase, pathToMiddlewareInfo} = require('./utils/str');
...
async function mvc(ctx, next) {
    ...
    if(cfg_app.app_debug) {
        ...
        ctx.request.body && Logger.http('Post:', ctx.request.body);
    }
    ...
    const controllerPaths = [...ctx.DEEP.split('/'), controller_folder];
    ...
    if(controller_folder == cfg_view.view_folder) {
        const content = await readFile(path.join(cfg_app.base_dir, controllerPaths.join('/'), ctx.CONTROLLER + cfg_view.view_depr + ctx.ACTION + cfg_view.view_ext));
        if(!content) {
