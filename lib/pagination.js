...
class Pagination extends Context
{
    ...
    // 首页
    _index() {
        return this._page == 1 ? '' : this._parseTpl('index_tpl', 1);
    }

    // 末页
    _end() {
        return this._page == this._totalPage ? '' : this._parseTpl('end_tpl', this._totalPage);
    }

    // 上一页
    _prev() {
        return this._page == 1 ? '' : this._parseTpl('prev_tpl', this._page - 1);
    }

    // 下一页
    _next() {
        return this._page == this._totalPage ? '' : this._parseTpl('next_tpl', this._page + 1);
    }
    ...
    _initRule() {
        ...
        if(url_index.slice(0, 1) == ':') {
            url_index = this.$url.ruleUrl(url_index.substr(1));
        }

        if(url_page.slice(0, 1) == ':') {
            url_page = this.$url.ruleUrl(url_page.substr(1));
        }
