...
class View extends Context {
    ...
    async render(content) {
        content = this._engine.render(content, this._data);
        return content;
    }
    ...
    async fetch(template) {
        ...
        const content = await this._engine(file_name, this._data || {});
        return content;
    }
    ...
    parsePath(template=this.ctx.ACTION) {
        ...
        view_file = path.join(this.ctx._.__node.path, './', view_file);
        return view_file;
    }
