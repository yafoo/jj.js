...
function pathToRouteInfo(path, default_deep, default_controller, default_action) {
    ...
    const bases = [...default_deep.split('/').filter(Boolean), default_controller, default_action];
    const paths = path.replace(/^\/|\/$/g, '').split('/').filter(Boolean).map(toUnderScore);
    if(paths.length > bases.length) {
        // Todo: 添加params参数
        throw new Error(`RouteError: 路径长度大于默认应用深度（default_deep长度+2）`);
    }
    paths.forEach(p => {
        if(!validatePath(p)) throw new Error(`RouteError: 路径不合法：${path}`);
    });
    const [ACTION, CONTROLLER, ...DEEPS] = bases.map((p, i) => paths[i] !== undefined ? paths[i] : p).reverse();

    const routeInfo = { DEEP: DEEPS.reverse().join('/'), CONTROLLER, ACTION };
...
function buildPathArray(extra, deep, controller, action) {
    ...
    const newers = extra.replace(/^\/|\/$/g, '').split('/').filter(Boolean).reverse().map(toUnderScore);
    return bases.map((val, i) => newers[i] !== undefined ? newers[i] : val);
}
