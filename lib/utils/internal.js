...
function buildPathArray(extra, deep, controller, action) {
    ...
    if(!extra.startsWith('/')) {
        ...
        extras[0] || (extras[0] = action);
        extras[1] || (extras[1] = controller);
        return extras;
    }
}
...
function getViewPath(template = '', deep, controller, action, view_folder, view_depr, view_ext) {
    ...
    pt.extname(template) || (template += view_ext);
    return template;
}
...
/**
 * @function - 获取路径对应的深度
 * @param {string} path - 路径，绝对地址
 * @param {string} base_dir - 基础目录，绝对地址
 * @returns {string} - 深度
 */
function pathToDeep(path, base_dir) {
    return pt.relative(pt.join(base_dir, 'app'), path).split(pt.sep).filter(Boolean).join('/');
}

module.exports = {toCamelCase, toUnderScore, pathToRouteInfo, getViewPath, getUrlPath, pathToMiddlewareInfo, pathToDeep};