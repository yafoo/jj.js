...
const {app: {base_dir}} = require('./config');
const {toCamelCase, pathToDeep} = require('./utils/parse');
...
class TypesGenerator {
    ...
    watch() {
        ...
        this.state._watcher = fs.watch(appDir, { recursive: true }, (eventType, filename) => {
            ...
            debounceTimer = setTimeout(() => {
                this.handleFileChange(eventType, filename, prevFile);
            }, DEBOUNCE_DELAY);
        });
     ...
     */
    handleFileChange(eventType, filename, prevFile) {
        if(!filename) return;

        const fullPath = path.join(this.appDir, filename);
 ...
 **/`);
    ...
    createProps($props) {
        ...
        Object.entries($props).forEach(([node_name, types]) => {
            ...
            if(libs.includes(node_name)) uni_types.unshift(`JJ${toCamelCase('_' + node_name)}`);
    ...
    createJJTypes() {
        return [
            '/**',
            ...libs.map(lib => ` * @typedef {import('jj.js/types').${toCamelCase('_' + lib)}} JJ${toCamelCase('_' + lib)}`),
            '*/'
        ].join('\n');
    }
     ...
     */
    async getNodeList(dir, ignore = ['types.js']) {
