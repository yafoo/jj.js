...
async function createFile(f) {
    ...
    try {
        // 安全地获取调用者模块路径
        // @ts-ignore - module.parent 在运行时存在，但类型定义中已弃用
        const callerModule = module.parent?.parent?.parent;
        if (callerModule && callerModule.filename) {
            ignore.push(path.basename(callerModule.filename));
        }
    } catch (e) {
        // 忽略获取调用者模块的错误
    }
