...
class TypesGenerator {
    ...
    watch() {
        ...
        try {
            ...
            if(e.code === 'ERR_FEATURE_UNAVAILABLE_ON_PLATFORM') {
                Logger.system('watch: recursive watch is not supported on this platform, types auto-generation is disabled');
            } else {
                throw e;
            }
        }
    ...
    async createFile(f) {
        ...
        const ignore = [];
        const node_list = await this.getNodeList(this.appDir, ignore);

        const file_content = await this.generateFileContent(node_list);
     ...
     */
    async generateFileContent(node_list) {
        /** @type {Object<string, string[]>} */
        const $props = {};
        const jj_types = this.createJJTypes();
        let types = this.createTypes(node_list, '', $props);
        const self_config_types = await this.createSelfConfigTypes($props);
        if(self_config_types) types += '\n\n' + self_config_types;
 ...
 **/`);
            ...
            const max_deep = this.state._deep ? toCamelCase('_' + this.state._deep.split('/').join('_')) : '';
            if(max_deep.startsWith(base_type)) {
     ...
     * 生成根目录自定义配置类型定义
     * @param {Object<string, string[]>} [prop_list] - $属性列表，接收返回值
     * @returns {Promise<string>} - [类型定义]
     */
    async createSelfConfigTypes(prop_list = {}) {
        const baseDir = BASE_DIR;
        const type_name = 'SelfConfig';
        let typestr = '';

        // 检查 config.js / config.json 文件
        for(const ext of ['config.js', 'config.json']) {
            const filePath = path.join(baseDir, ext);
            if(fs.existsSync(filePath)) {
                const node_name = path.parse(ext).name; // 'config'
                const importPath = './' + ext;
                typestr += `/**\n * @typedef {typeof import('${importPath}')} ${type_name}\n */`;
                if(!prop_list[node_name]) prop_list[node_name] = [];
                prop_list[node_name].push(type_name);
                break; // config.js 和 config.json 只取一个
            }
        }
        ...
        // 检查 config/ 文件夹
        const configFolderPath = path.join(baseDir, 'config');
        if(fs.existsSync(configFolderPath) && fs.statSync(configFolderPath).isDirectory()) {
            const node_list = await this.getNodeList(configFolderPath, [
                'app.js', 'view.js', 'db.js', 'log.js', 'cache.js', 'page.js', 'routes.js', 'cookie.js', 'tpl.js'
            ]);
            if(Object.keys(node_list).length) {
                typestr = this.createTypes(node_list, type_name);
                if(!prop_list['config']) prop_list['config'] = [];
                prop_list['config'].push(type_name);
            }
        }
    ...
    async getNodeList(dir, ignore = []) {
        ...
        for(const dirent of files) {
            ...
            if(file_type == 'file') {
                ...
                type_list[rel_path] = {node_name, file_name, file_type, children: await this.getNodeList(abs_path, ignore)};
            }
        }
        return type_list;
    }
