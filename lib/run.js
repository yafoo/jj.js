...
async function run(ctx, next, control_type=cfg_app.controller_folder) {
    ...
    // 模版文件内容直接输出
    if(control_type == cfg_app.view_folder) {
        const content = await readFile(path.join(ctx.$[ctx.APP][control_type].__node.path, ctx.CONTROLLER + cfg_app.view_depr + ctx.ACTION + cfg_app.view_ext));
        if(!content) {
            throw new Error(`模板文件:${ctx.APP}/${control_type}/${ctx.CONTROLLER + cfg_app.view_depr + ctx.ACTION + cfg_app.view_ext}不存在！`);
        }
        ctx.body = content;
        return;
    }
    ...
    // 控制器
    const Controller = ctx.$[ctx.APP][control_type][ctx.CONTROLLER] || ctx.$[ctx.APP][control_type]['empty'];

    if (ctx.CONTROLLER[0] == '_' || typeof Controller != 'function') {
        if(cfg_app.app_debug) {
    ...
    if (ctx.ACTION[0] == '_' || typeof $controller[action] != 'function') {
        if(cfg_app.app_debug) {
