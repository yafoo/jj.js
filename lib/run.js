...
const {app: cfg_app, db, page, log, cache, cookie, tpl} = require('./config');
...
async function run(ctx, next, control_type=cfg_app.controller_folder) {
    // 设置根加载器
    ctx.$ = loader('./', ctx);
    ctx._ = loader(path.dirname(module.parent.parent.parent.parent.filename), ctx);
    ...
    if(control_type == cfg_app.view_folder) {
        const content = await readFile(path.join(ctx._[ctx.APP][control_type].__node.path, ctx.CONTROLLER + cfg_app.view_depr + ctx.ACTION + cfg_app.view_ext));
        if(!content) {
