            ...
            return new Proxy({__proto__: arguments[2].prototype}, {
                get: (target, $prop, receiver) => {
                    ...
                    if(_[APP] && _[APP][$prop] === undefined) {
                        ...
                        // lib内有调用，防止覆盖
                        if(['logger', 'pagination', 'request', 'response', 'url', 'view'].includes(prop) && _[APP][$prop] && _[APP][$prop].__node && _[APP][$prop].__node.type != 'class' && $[prop]) {
                            _[APP][$prop] = $[prop];
                        }
                    }

                    // 屏蔽部分lib
                    return _[APP] && _[APP][$prop] || _[prop] || (!['app', 'loader', 'router', 'run', 'storage', 'types'].includes(prop) && $[prop]) || undefined;
                }
            });
        }
});
