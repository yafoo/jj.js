...
class Db extends Context
{
    ...
    async select(condition) {
        ...
        if(this._options.cache_time) {
            ...
            if(this.cacheInstance.get(cache_key)) {
                this.reset();
                return this.cacheInstance.get(cache_key);
            }
            const result = await this.query(this._queryStr, params);
            this.cacheInstance.set(cache_key, result, cache_time);
            return result;
        }
    ...
    async pagination({page, page_size, pagination} = {}) {
        ...
        !pagination && (pagination = this.paginationInstance);
     ...
     */
    // @ts-ignore
    static _cache = new (require('./cache'))();
    static {
        this._cache.setIntervalTime(20 * 60 * 60);
    }
