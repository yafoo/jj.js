...
class Db extends Context
{
    ...
    connect(config='default') {
        this._config = typeof config === 'string' ? (require('./config').db[config] || config) : config;
    ...
    /**
     * 按逗号分割字段字符串，但忽略括号内的逗号
     * @private
     * @param {string} str - 字段字符串
    ...
    _splitField(str) {
        ...
        for(let i = 0; i < str.length; i++) {
            ...
            if(ch === '(') depth++;
                ...
                result.push(current.trim().replace(/ +/g, ' ').replace(/ as /g, ' '));
                current = '';
                continue;
            }
            current += ch;
    ...
    /**
     * 过滤SQL片段中的危险字符，防止拼接进group/having子句时发生SQL注入
     * @private
     * @param {string} fragment
     * @returns {string}
     */
    _sanitizeSqlFragment(fragment) {
    ...
    async select(condition) {
        ...
        const group = this._sanitizeSqlFragment(this._options.group);
        const having = this._sanitizeSqlFragment(this._options.having);
        const limit = this._options.limit;

        this._queryStr = [
            'select', distinct, field, 'from', table, join, where[0], group, having, order, limit
        ].join(' ');
    ...
    async insert(data) {
        ...
        this._queryStr = `INSERT_PROBE into ${table} ${sql}`;
        if(this._options.getSql) {
            this._options.getSql = false;
            this.sql = this._sql.format(this._queryStr, params);
            return;
    ...
    async tableInfo(table) {
        ...
        const tableName = (this._options.prefix + this._table).replace(/[^a-zA-Z0-9_]/g, '');
        if(this._options.cache_time) {
            const cache_time = this._options.cache_time;
    ...
    _parseField() {
        ...
        this._options.field.forEach(value => {
            ...
            if(!~field_str.indexOf(')')) {
                field_str = '`' + field_str.replace(/\./g, '`.`') + '`';
            }
            field_str = field_str.replace('`*`', '*');
            fields.push(field_str + alias);
        });
        return fields.join(',') || '*';
    }
