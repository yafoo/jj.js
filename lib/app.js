...
const {app: cfg_app, cookie: cfg_cookie} = config;
...
class App extends Koa
{
    ...
    _init(options) {
        ...
        // static
        const {static_dir, options: static_options} = typeof cfg_app.static_dir === 'object' ? cfg_app.static_dir : {static_dir: cfg_app.static_dir, options: {}};
        cfg_app.static_dir && this.use(require('koa-static')(path.join(cfg_app.base_dir, static_dir || ''), static_options || {}));
        ...
        // cookie
        cfg_cookie.keys && (this.keys = cfg_app.app_debug ? [new Date().setMinutes(0, 0, 0).toString()] : cfg_cookie.keys);
