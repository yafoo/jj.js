...
/**
 * @typedef {import('../../types').MongodbConfig} MongodbConfig
...
class Mongodb extends Sql
{
     ...
     * 连接数据库
     * @public
     * @param {MongodbConfig} config - 数据库配置标识或连接参数
    ...
    async connect(config) {
        ...
        if(!client) {
            ...
            try {
                ...
                throw new Error('Missing dependency: mongodb. Please install it: npm install mongodb');
            }
            const { MongoClient } = mongodb;
            // 处理无认证场景：user/password为空时不拼接认证信息
            let url;
            if(config.user && config.password) {
                url = `mongodb://${config.user}:${config.password}@${config.host}:${config.port || 27017}`;
            } else {
                url = `mongodb://${config.host}:${config.port || 27017}`;
            }
            client = new MongoClient(url);
            clients.set(this._config, client);
            this.logger.sql(`MONGODB数据库连接创建成功：{all: ${clients.size}}`);
            await client.connect();
            this.logger.sql(`MONGODB数据库连接成功`);
        }
    ...
    async _handleInsert(sql, params, collection) {
        ...
        if(valuesMatch) {
            ...
            if(fieldsMatch) {
                const fields = fieldsMatch[1].split(',').map(field => field.trim().replace(/`/g, ''));
    ...
    async _handleUpdate(sql, params, collection) {
        ...
        if(setMatch) {
            ...
            setFields.forEach((field, index) => {
                ...
                const fieldName = key.trim().replace(/`/g, '');
                // SET 中的值如果是占位符?，从params前部取值
                if(value && value.trim() === '?') {
                    update[fieldName] = params[index];
                } else if(value) {
                    // 字面量值
                    const v = value.trim().replace(/^'|'$/g, '');
                    update[fieldName] = !isNaN(Number(v)) ? Number(v) : v;
                }
            });
            
            // 解析更新条件：WHERE参数从params中SET参数之后开始
            const whereMatch = sql.match(/where\s+([\s\S]+)/i);
            /** @type {Record<string, any>} */
            let filter = {};
            if(whereMatch) {
                const whereParams = params.slice(setParamCount);
                filter = this._parseWhere(whereMatch[1], whereParams);
            }
