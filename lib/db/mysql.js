...
/**
 * @typedef {import('../../types').MysqlConfig} MysqlConfig
...
class Mysql extends Sql
{
     ...
     * 连接数据库连接池
     * @public
     * @param {MysqlConfig} config - 数据库配置标识或连接参数
    ...
    async connect(config={}) {
        ...
        if(!pool.has(this._config)) {
            ...
            try {
                ...
                throw new Error('Missing dependency: mysql. Please install it: npm install mysql');
            }
            const cur_pool = mysql.createPool(this._config);
