...
/**
 * @typedef {import('../../types').SqliteConfig} SqliteConfig
...
class Sqlite extends Sql
{
     ...
     * 连接数据库
     * @public
     * @param {SqliteConfig} config - 数据库配置标识或连接参数
    ...
    async connect(config) {
        ...
        if(!connection) {
            ...
            try {
                ...
                throw new Error('Missing dependency: sqlite3. Please install it: npm install sqlite3');
            }
            ...
            if(this._config.optimize) {
                ...
                this.logger.sql(`SQLITE数据库连接已开启性能优化：journal_mode = WAL; synchronous = NORMAL; busy_timeout: 5000`);
            }
        }
