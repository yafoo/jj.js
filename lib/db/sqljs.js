...
/**
 * @typedef {import('../../types').SqljsConfig} SqljsConfig
...
class Sqljs extends Sql
{
    ...
    async _initSqlJs() {
        ...
        try {
            ...
            throw new Error('Missing dependency: sql.js. Please install it: npm install sql.js');
        }
    ...
    _save() {
        ...
        try {
            const data = conn.export();
            fs.writeFileSync(dbPath, data);
            this.logger.sql(`SQLJS数据库已保存到文件：${dbPath}`);
        } catch(e) {
     ...
     * 连接数据库
     * @public
     * @param {SqljsConfig} config - 数据库配置标识或连接参数
    ...
    async connect(config) {
        ...
        if(!connection) {
            ...
            if(database === ':memory:') {
                ...
                try {
                    if(fs.existsSync(database)) {
                        ...
                        this.logger.sql(`SQLJS数据库已从文件加载：${database}`);
                    } else {
                        connection = new SQL.Database();
                    }
            ...
            if(config.optimize) {
                ...
                try { connection.run('PRAGMA journal_mode = WAL;'); optimizations.push('journal_mode = WAL')} catch(e) {}
                // 性能优化：配合WAL模式，减少不必要的磁盘同步
                try { connection.run('PRAGMA synchronous = NORMAL;'); optimizations.push('synchronous = NORMAL')} catch(e) {}
                // 性能优化：设置锁等待超时时间
                try { connection.run('PRAGMA busy_timeout = 5000;'); optimizations.push('busy_timeout = 5000')} catch(e) {}
            }

            connections.set(this._config, connection);
            this.logger.sql(`SQLJS数据库连接创建成功：{all: ${connections.size}}`);
            optimizations.length && this.logger.sql(`SQLJS数据库连接已开启性能优化：${optimizations.join('; ')}`);
        }
