...
```javascript
const libs = ['ctx', 'context', 'cache', 'cookie', 'db', 'logger', 
    ...
    construct() {
        return new Proxy({__proto__: arguments[2].prototype}, {
            get: (target, $prop, receiver) => {
                // 1. 如果属性已存在，直接返回
                if($prop in target || typeof $prop == 'symbol' || 
                   $prop == 'inspect' || $prop == 'ctx' || $prop == '$next') {
                ...
                return _[APP][$prop] = new Proxy(function() {}, {
                    ...
                    get: (...args) => {
                        return _[APP][prop] && _[APP][prop][args[1]]
...
function loader(path, ...args) {
    ...
    infoStore.set(node, {path: dirPath, type: dirType});
    ...
    function creatLoader(node) {
        return new Proxy(node, {
            get: (node, prop, receiver) => {
                ...
                infoStore.set(_node, {path: _nodePath, type: _nodeType});
...
async function mvc(ctx, next) {
    ...
    const action = typeof $controller[humpAction] == 'function' 
                   && !$controller[humpAction].__ISCLASS__
                   ? humpAction 
                   : '_empty';
...
## 🔄 近期重要变更（06-15 至 06-24）

### 核心变更

#### 1. 运行层文件重命名：`run.js` → `mvc.js`

- **变更前**：`lib/run.js` 负责控制器执行逻辑
- **变更后**：`lib/mvc.js` 负责控制器执行逻辑
- **影响**：文件命名更清晰地表达其作为 MVC 中间件的角色

#### 2. Controller 和 Middleware 引入懒加载实例化机制

**Controller 类新增懒加载属性：**

```javascript
// 私有缓存属性
__view = null;        // View 实例缓存
__response = null;    // Response 实例缓存
...
**优势：**
- 按需创建实例，减少不必要的对象创建
- 支持自定义注入（可通过 `_$view` 和 `_$response` setter 覆盖）
- 提高性能，特别是那些不需要视图或响应操作的控制器方法

#### 3. Ctx 类 Proxy 实现优化

**变更内容：**
- 新增 `libs` 数组明确定义允许 `$` 访问的系统类库列表
- 优化了属性查找逻辑，更清晰地处理 `$` 和 `_` 快捷访问
- 改进了 Class 实例化判断，使用 `__ISCLASS__` 标记
...
**变更内容：**
- `__node` 改为 `__NODE__`（大写）
- 新增 `__ISCLASS__` 标记支持
- 使用 `Map` 替代普通对象存储 `infoStore` 和 `typeStore`
- 新增 `set` 拦截器，支持设置属性到 Class 实例
- 路径处理使用 `pt.normalize()` 和正则替换
...
**变更内容：**
- 增加 `!$controller[humpAction].__ISCLASS__` 判断
- 确保方法不是 Class 类型
...
| 懒加载实例化 | Getter/Setter 按需创建 | 减少不必要的对象创建，提高性能 |
